...
{
public:
// Implementation of k-core decomposition algorithm of Batagelj and Zaversnik (https://arxiv.org/abs/cs/0310049)
    ...
    static std::shared_ptr<graph_access> createCoreGraph(k_cores kCores, NodeID k, std::shared_ptr<graph_access> G) {
        ...
        if (core_graph->getMinDegree() != min_degree) {
            LOG1 << "ERROR: Wrong min degree " << core_graph->getMinDegree()
                 << " instead of " << min_degree;
            exit(1);
        }
