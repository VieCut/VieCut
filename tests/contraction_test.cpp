...
TEST(ContractionTest, NoContr) {
    ...
    std::shared_ptr<graph_access> cntr = contraction::contractGraph(G, mapping, G->number_of_nodes(), reverse_mapping);
...
TEST(ContractionTest, ContrBlock) {
    ...
    for (std::string graph : graphs) {
        ...
        std::shared_ptr<graph_access> cntr = contraction::contractGraph(G, mapping, 2, reverse_mapping);
